...
public class BusinessVerification {
    ...
    public static main(String[] args) {
        ...
        PartnerParams params = new PartnerParams(JobType.BUSINESS_VERIFICATION, "<unique ID for user>", "< unique job ID >", optionalInfo);

        // Create ID number info
        IdInfo idInfo = new IdInfo("<2 letter country code>", "< id type>", "< valid id number >", " < business type>", "< postal code>");

        // Options for the job
        boolean returnJobStatus = false; // Set to true if you want to get the job result in sync (in addition to the result been sent to your callback). If set to false, result
        // is sent to callback url only
        boolean returnHistory = false; // Set to true to receive all of the updates you would otherwise have received in your callback as opposed to only the final result. You
        // must set return_job_status to true to use this flag.
        boolean returnImageLinks = false; // Set to true to receive links to the selfie and the photo it was compared to. You must set return_job_status to true to use this flag.
